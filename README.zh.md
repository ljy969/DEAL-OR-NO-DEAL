...
- **核心玩法：** 26 个箱子、9 个开箱回合（每轮依次打开 6/5/4/3/2/1/1/1/1 个
  其他箱子，共 24 个），前 8 个回合结束后银行家各出一次价，最后进入“交换箱子”
  终局，在“你的箱子”与“最后剩下的一个箱子”之间做决策。
...
> **说明：** 银行家在前 8 个开箱回合结束后各出一次价；第 9 个（最后一个）开箱回合结束后
> 不再出价，游戏直接进入到“交换箱子”终局。
...
- `acceptDeal()` / `rejectDeal()` / `keepCase()` / `switchCase()`——结束本局并
  捕获 `finalWinnings` 与“决策时刻”的期望值。
...
错误会被静默吞掉（例如上下文无法启动），因此音效绝不会阻塞游戏。
